const View = {
    ...
    updateLobby: (code, users) => {
        ...
        document.getElementById('participants-list').innerHTML = users.map(u => `<li>${u}</li>`).join('');
    },
    renderCard: (rest, remaining) => {
        if(!rest) return;
        ...
        // Iframe simple de mapa
        const query = encodeURIComponent(rest.name + " restaurant");
        document.getElementById('card-map-frame').src = `https://maps.google.com/maps?q=${query}&t=&z=15&ie=UTF8&iwloc=&output=embed`;
    },
    toggleFlip: () => {
    ...
    renderMatches: (matches) => {
        ...
        if(matches.length === 0) div.innerHTML = "<p>Esperando coincidencias...</p>";
        else div.innerHTML = matches.map(m => `<div style="padding:10px; border-bottom:1px solid #ccc"><strong>${m.name}</strong></div>`).join('');
    }
};